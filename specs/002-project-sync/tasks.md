...
**Goal**: Owner marks a project shared, issues a collaborator a token, collaborator's instance receives the full existing history.

**Independent Test**: Run quickstart.md steps 1-4.

### Implementation for User Story 1

- [ ] T011 [P] [US1] Implement `POST /sync/push` and `GET /sync/pull` handlers in `crates/aurelius-sync-server/src/routes.rs`, delegating to `sync::merge` (depends on T008, T007)
- [ ] T012 [P] [US1] Implement bearer-token auth extractor in `crates/aurelius-sync-server/src/auth.rs`: looks up `collaborator_grants` by token, checks `project` matches and `revoked_at IS NULL`, returns `401`/`403` per contracts/sync-api.md
...
- [ ] T015 [US1] Add `au sync enable <project> --server <url> (--issue-token <email> | --token <token>)`: creates/updates the `sync_config` row, and either inserts a new `collaborator_grants` row + prints the issued token (owner side) or stores the given token (collaborator side), then performs the initial bootstrap pull — `crates/au/src/main.rs` + `crates/au/src/commands.rs` (depends on T004, T007, T013)
