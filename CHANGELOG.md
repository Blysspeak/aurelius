...
### Added
- **Напоминания: у задачи появился срок, а у срока — адресат и подотчётная история переносов.** Задачник различал только «в очереди»/«в работе»/«закрыта» — момента, когда что-то должно случиться, у него не было вовсе, и обещание себе или ИИ жило лишь в памяти того, кто его дал. Теперь `reminders`/`reminder_events` — отдельная таблица с явным `state` (`pending`/`delivered`/`done`/`cancelled`, где `delivered` — не исход, а промежуточная остановка до `done`/`cancelled`, и спутать «сделано» с «отменено» об одном nullable-поле стало нельзя) и журналом переносов: `snooze_count`/`original_due_at` на самой строке говорят, сколько раз и откуда, а `reminder_events` — когда именно случился каждый перенос. Срок живёт на самой задаче (`TaskFields.due_at`, седьмое поле без единой правки схемы) — `au task new --due` и `au task update --due` заводят или двигают привязанное напоминание тем же `snooze`, так что сдвиг дедлайна остаётся в общем журнале, а не теряется отдельной операцией мимо истории. Адресат (`owner`: `me` / `ai` / `both`) решает, каким каналом можно доставить — напоминание для человека не тратится на сессию, которая не в силах на него отреагировать, и наоборот. Часами владеет ровно один процесс — постоянно работающий демон под systemd, а не MCP-сервер: сервер поднимается заново на каждую сессию Claude Code, и таймер внутри него размножился бы по числу открытых сессий, а не остался одним — этот вариант обсуждался с владельцем и отвергнут явно. Доставка в уже открытую сессию обходится вовсе без таймера: два хука сессии, `Stop` и `UserPromptSubmit`, читают ту же таблицу ровно в момент, когда сессия и так уже на связи, и помечают напоминание доставленным условным `UPDATE` вместо отдельного чтения с последующей записью — иначе два конкурирующих потребителя забрали бы одно и то же напоминание дважды. Ставить и разбирать напоминания можно и с CLI (`au remind add/list/show/done/cancel/snooze`), и из MCP (`reminder_add/list/show/done/snooze`) — значит, напоминание может поставить не только Влад, но и модель, не выходя из разговора в отдельный вызов бинаря.
...
### Fixed
- Обход графа в memory_recall и memory_context получил предел в 200 узлов и отсечку глубины на 3: явный depth=2 больше не разворачивает ответ в мегабайты через хаб-узлы; обрезание честно сообщается в блоке truncation.
