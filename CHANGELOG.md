...
### Added
- **Бинарник `aurelius` отвечает на `--version` и `--help`.** `main` не разбирал аргументы вообще: любой флаг игнорировался и на stdio поднимался MCP-демон, поэтому `aurelius --version` вис, удерживая stdin, и узнать версию установленной сборки было нечем. Теперь аргументы разбираются до `tracing_subscriber::fmt::init` — иначе строки лога на stderr перемешиваются с ответом — и `-V`/`--version` печатают версию крейта, а `-h`/`--help` — короткую справку.

### Fixed
- Перевели все отслеживаемые текстовые файлы репозитория из CRLF в LF и закрепили eol=lf в .gitattributes: speckit-скрипты больше не падают на Linux с «env: bash\r: Нет такого файла или каталога».

### Changed
- memory_status теперь компактный по умолчанию: заметки обрезаются до выдержки с флагом note_truncated, улики сводятся к total/green/last_green, скрытые элементы считаются в блоке truncation; прежний полный ответ возвращает full=true.

## [v3.4.4] — 2026-09-06

### Fixed
- **install.sh обновляет уже установленный плагин.** `claude plugin install` для установленного плагина завершается кодом 0 с текстом «already installed» и ничего не обновляет, поэтому ветка обновления никогда не выполнялась: после релиза 3.4.3 в кэше оставался плагин 3.4.2 с блоком `mcpServers`. Теперь скрипт читает вывод установки и при «already installed» вызывает `claude plugin update aurelius`

## [v3.4.3] — 2026-09-06

### Changed
- **MCP-сервер регистрируется в пользовательской области, а не внутри плагина.** Серверам, объявленным в `plugin.json`, Claude Code даёт инструментам имена `mcp__plugin_aurelius_aurelius__*`, и каждая ссылка на `mcp__aurelius__*` — в карточках au, шаблонах нарядов ulika, `CLAUDE.md` и правилах разрешений — переставала совпадать. Теперь `install.sh` регистрирует сервер командой `claude mcp add -s user aurelius au mcp` после миграции старых записей (повторный запуск ничего не меняет), миграция не трогает такую запись в `~/.claude.json`, а плагин несёт только хуки, карточки и `/pickup`. У `marketplace.json` появилось описание — `claude plugin validate` больше не предупреждает

## [v3.4.2] — 2026-09-05

### Fixed
- **Плагин переехал в каталог `plugin/`.** Корнем плагина был корень репозитория, а `claude plugin install` копирует корень плагина целиком в свой кэш — вместе с `target/` на десятки гигабайт установка не завершалась и съедала память. Теперь манифест лежит в `plugin/.claude-plugin/plugin.json`, маркетплейс указывает source `./plugin`, а тест `plugin_root_stays_small` сторожит размер каталога плагина

## [v3.4.1] — 2026-09-05

### Fixed
- **`install.sh` 3.4.0 падал на первом же бинарнике.** В `install_binary` три `local` в одной строке: bash раскрывает все слова команды до присваивания, и под `set -u` ещё не заданная `name` обрывала скрипт с «unbound variable» — ни бинарники, ни плагин, ни миграция не успевали выполниться. `bash -n` этого не видит: ошибка времени выполнения. Объявление разбито на две строки; функция теперь проверяется реальным запуском во временном каталоге
